We conducted a hypothesis test. Null hypothesis $H_0$ assumes that lottery numbers are sampled uniformly,
alternative hypothesis $H_A$ assumes the opposite. We used Pearson's $\chi^2$ test, a commonly recommended
test for the probabilities of observing categorical data (\todo{sOuRcE?}). Furthermore, it is the same type
of test performed by Drakakis et al.~\cite{Drakakis}, which allows us to check our (and their) results.
% We can also satisfy the assumptions of the $\chi^2$ test, and 
...
poor job of imitating the random choices that occur under the null hypothesis. Boland and Pawitan \todo{cite} showed
...
an observed discrete distribution. We computed the expected distribution using the following formula proved by Drakakis (2007) \todo{cite}.
